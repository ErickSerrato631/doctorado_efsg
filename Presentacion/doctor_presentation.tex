...
\begin{frame}{Variables y Allee fuerte}
  ...
  \]
  \vfill
  \textbf{ODE} (límite homogéneo del PDE), con el mismo \(\mu\in\{0,1\}\) \cite{johnson2019allee}:
  \begin{align*}
    \dot{c} &= r_c c f_{\mathrm{Allee}}(c) - c(\alpha s^2 + \beta i^2) - \mu c(\gamma s^2 + \eta i^2),\\
    \dot{s} &= r_s s\left(1-\tfrac{s}{k_s}\right) - \gamma c^2 s + \delta i^2 s - \tfrac{\mu}{2}\alpha c^2 s,\\
    \dot{i} &= r_i i\left(1-\tfrac{i}{k_i}\right) + \delta s^2 i - \eta c^2 i - \tfrac{\mu}{2}\beta c^2 i.
  \end{align*}
\end{frame}
