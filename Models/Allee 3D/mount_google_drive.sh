...
if command -v mountpoint >/dev/null 2>&1 && mountpoint -q "${MOUNT_POINT}"; then
  ...
  echo "  Django (rd_coupled_lab): coloca steady_states_full_run.json en «${MOUNT_POINT}/Resultados paper/estados_estacionarios/»"
  echo "  o exporta STEADY_STATES_CATALOG_JSON / GOOGLE_DRIVE_MOUNT_POINT. Si montaste en /root/…, ejecuta Django como root o remonta en \$HOME/googledrive como tu usuario."
  exit 0
fi
