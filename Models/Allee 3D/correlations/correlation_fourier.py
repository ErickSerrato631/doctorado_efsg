...
def load_parameters():
    ...
    return T, dt, nb, save_images, nodes_in_xaxis, nodes_in_yaxis, nodes_in_zaxis, space_size, dx, sample_rate
...
def validate_matrices(nueva_ruta, T, dt, nb):
    ...
    matrix_files = [f for f in os.listdir(matrices_dir) if f.startswith('matrix_') and f.endswith('.npz')]
    if not matrix_files:
    ...
    for field_name in field_names:
        for t_val in np.arange(0, T + dt, dt):
            t_str = f"{t_val:.3f}"
            matrix_file = os.path.join(matrices_dir, f"matrix_{field_name}_{t_str}_nb_{nb}.npz")
            legacy_matrix_file = os.path.join(matrices_dir, f"matrix_{field_name}_{t_str}_nb_{nb}.txt")
            if os.path.exists(matrix_file) or os.path.exists(legacy_matrix_file):
...
def _load_field_volume(field_name, t, block):
    matrices_dir = os.path.join(os.getcwd(), 'matrices')
    npz_file = os.path.join(matrices_dir, f"matrix_{field_name}_{t:.3f}_nb_{block}.npz")
    txt_file = os.path.join(matrices_dir, f"matrix_{field_name}_{t:.3f}_nb_{block}.txt")
    if os.path.exists(npz_file):
        with np.load(npz_file) as data:
            return np.asarray(data["field"], dtype=float), npz_file
    if os.path.exists(txt_file):
        return np.loadtxt(txt_file, float), txt_file
...
def power_spectrum(field_name, t, block, save_image=False, save_images='N'):
    ...
    """
    try:
        field, field_file = _load_field_volume(field_name, t, block)
    except FileNotFoundError:
        print(f"⚠ Archivo no encontrado: matrix_{field_name}_{t:.3f}_nb_{block}.npz")
        return None, None
    except Exception as e:
        print(f"Error al cargar el archivo {field_file}: {e}")
        return None, None

    # FFT nD centrada (3D para los volúmenes nuevos)
    ...
    center = tuple(n // 2 for n in fft_magnitude.shape)
    fft_magnitude[center] = 0
    ...
    if save_image and save_images == 'Y':
        try:
            ...
            im = ax.imshow(_central_slice(fft_magnitude), cmap='inferno', origin='lower')
...
def correlation_function_fourier(dft_field1, dft_field2, field1_name, field2_name, step, block):
    ...
    min_shape = tuple(min(a, b) for a, b in zip(fft1.shape, fft2.shape))
...
def inverse_correlation_fft(centered_correlation, field1_name, field2_name, step, block):
    ...
    inverse_fft = np.fft.ifftn(np.fft.ifftshift(centered_correlation)).real  # Obtener solo la parte real
...
def compute_correlation_length(centered_inverse_fft, dx):
    ...
    shape = centered_inverse_fft.shape
    centers = tuple(n // 2 for n in shape)
    indices = np.indices(shape)
    r_sq = np.zeros(shape, dtype=float)
    for axis, center in enumerate(centers):
        r_sq += (indices[axis] - center) ** 2
    r = np.sqrt(r_sq).astype(int)

    radial_sum = np.bincount(r.ravel(), weights=centered_inverse_fft.ravel())
    counts = np.bincount(r.ravel())
    radial_profile = radial_sum / np.where(counts == 0, 1, counts)
...
if __name__ == "__main__":
    ...
    # Cargar parámetros
    T, dt, nb, save_images, nodes_in_xaxis, nodes_in_yaxis, nodes_in_zaxis, space_size, dx, t_step = load_parameters()
