...
def _central_slice(arr: np.ndarray) -> np.ndarray:
    if arr.ndim == 3:
        return arr[:, :, arr.shape[2] // 2]
    return arr


def _slice_components(*arrays: np.ndarray) -> Tuple[np.ndarray, ...]:
    return tuple(_central_slice(a) for a in arrays)
...
def sigma_plus_density(
    ...
    gc = calculate_gradient_nd(c, dx)
    gs = calculate_gradient_nd(s, dx)
    gi = calculate_gradient_nd(i, dx)
    return Dc * sum(g * g for g in gc) + Ds * sum(g * g for g in gs) + Di * sum(g * g for g in gi)


def sigma_plus_integral(sigma: np.ndarray, dx: float) -> float:
    return float(np.sum(sigma) * (dx ** sigma.ndim))
...
def process_one_time(
    ...
    mag_c = np.sqrt(sum(g * g for g in gc))
    mag_s = np.sqrt(sum(g * g for g in gs))
    mag_i = np.sqrt(sum(g * g for g in gi))
    ...
    ts = f"{t:.3f}"
    c2, s2, i2 = _slice_components(c, s, i)
    mag_c2, mag_s2, mag_i2 = _slice_components(mag_c, mag_s, mag_i)
    mag_Jc2, mag_Js2, mag_Ji2 = _slice_components(mag_Jc, mag_Js, mag_Ji)
    sigma2 = _central_slice(sigma)
    Jcx2, Jcy2, Jsx2, Jsy2, Jix2, Jiy2 = _slice_components(Jcx, Jcy, Jsx, Jsy, Jix, Jiy)

    plot_three_panels(
        (mag_c2, mag_s2, mag_i2),
        (r"$\|\nabla c\|$", r"$\|\nabla s\|$", r"$\|\nabla i\|$"),
        r"$|\nabla\phi|$",
        rf"Gradient magnitudes, central z-slice ($t={ts}$)",
        ext,
        out_dir / f"grad_mag_t_{ts}.png",
        dpi,
    )
    plot_three_panels(
        (mag_Jc2, mag_Js2, mag_Ji2),
        (r"$\|\mathbf{J}_c\|$", r"$\|\mathbf{J}_s\|$", r"$\|\mathbf{J}_i\|$"),
        r"$|\mathbf{J}|$",
        rf"Diffusive flux magnitudes, central z-slice ($t={ts}$)",
        ext,
    ...
    im = ax.imshow(sigma2, origin="lower", extent=ext, aspect="equal", cmap="magma")
    ax.set_title(
        r"$\sigma^{+}=\sigma_{\mathrm{diss,tot}}$" + rf" ($t={ts}$)"
    )
    ax.set_xlabel("x")
    ax.set_ylabel("y")
    sig_vmin, sig_vmax = _scalar_color_limits(sigma2)
    ...
    if do_quiver:
        c_vmin, c_vmax = _log_field_color_scale("c", ts, c2)
        plot_quiver_flux_on_scalar(
            ...
            dpi,
            title=rf"$\mathbf{{J}}_c$ on $c$ central z-slice ($t={ts}$)",
            quiver_color="coral",
            bg_cmap=_CMAP_FIELD,
            bg_vmin=c_vmin,
            bg_vmax=c_vmax,
        ...
        s_vmin, s_vmax = _log_field_color_scale("s", ts, s2)
        plot_quiver_flux_on_scalar(
            s2,
            Jsx2,
            Jsy2,
            ...
            dpi,
            title=rf"$\mathbf{{J}}_s$ on $s$ central z-slice ($t={ts}$)",
        ...
        i_vmin, i_vmax = _log_field_color_scale("i", ts, i2)
        plot_quiver_flux_on_scalar(
            i2,
            Jix2,
            Jiy2,
            ...
            dpi,
            title=rf"$\mathbf{{J}}_i$ on $i$ central z-slice ($t={ts}$)",
    ...
    if do_control_plots and params.use_adaptive_control:
        ...
        u2 = _central_slice(u_field)
        u_vmin, u_vmax = _log_field_color_scale("u (Hill)", ts, u2)
        ...
        if do_quiver:
            gu = calculate_gradient_nd(u_field, dx)
            gux, guy = _slice_components(gu[0], gu[1])
            plot_quiver_flux_on_scalar(
                u2,
                gux,
                guy,
