...
RESULTS_DIMENSION_SUBDIR = os.getenv("ALLEE_RESULTS_SUBDIR", "3D").strip()


def _append_results_subdir(path: Path) -> Path:
    """Agrupa las salidas de este proyecto bajo Resultados Paper/3D por defecto."""
    if not RESULTS_DIMENSION_SUBDIR:
        return path
    if path.name == RESULTS_DIMENSION_SUBDIR:
        return path
    return path / RESULTS_DIMENSION_SUBDIR
...
def get_results_dir_from_env() -> Optional[Path]:
    ...
    if not env:
        return None
    try:
        root = _append_results_subdir(Path(env).expanduser()).resolve()
...
def get_results_dir_cloud_only() -> Path:
    ...
    if env:
        try:
            p = _append_results_subdir(Path(env).expanduser()).resolve()
    ...
    resultados_dir = _append_results_subdir(doctorado_dir / "Resultados Paper")
    try:
...
def get_results_dir(base_dir: Optional[Path] = None) -> Path:
    ...
    No hay fallback local: en este proyecto las salidas deben ir a Drive.
    
    Args:
        base_dir: Se conserva por compatibilidad de firma; no habilita fallback local.
    ...
    if env_results_dir:
        results_path = _append_results_subdir(Path(env_results_dir).expanduser())
        try:
            results_path.mkdir(parents=True, exist_ok=True)
            return results_path.resolve()
        except OSError as e:
            raise RuntimeError(f"RESULTS_DIR no accesible ({env_results_dir}): {e}") from e
    ...
    if mounted_drive is not None:
        ...
        drive_results_dir = _append_results_subdir(mounted_drive / "Doctorado Erick Serrato" / "Resultados Paper")
        ...
        try:
            ...
            raise RuntimeError(f"Error al acceder a Google Drive: {e}") from e

    raise RuntimeError(
        "No hay destino de resultados en Drive. Monta Google Drive con "
        "`bash mount_google_drive.sh` o define RESULTS_DIR apuntando a la carpeta de Drive."
    )
...
def ensure_results_dir_ready(base_dir: Optional[Path] = None) -> Path:
    ...
    if not explicit and under_drive and not is_google_drive_mounted():
        raise RuntimeError(
            ...
            "Ejecuta desde Allee: bash mount_google_drive.sh o define RESULTS_DIR apuntando a Drive."
        )
    ok, err = verify_results_dir_write_access(results_dir)
    if not ok:
        raise RuntimeError(f"No se puede escribir en {results_dir}: {err}")
