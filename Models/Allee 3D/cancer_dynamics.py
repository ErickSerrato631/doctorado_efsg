...
# Mallado / tiempo
spatial_dim = int(os.getenv('SPATIAL_DIM', '3'))
if spatial_dim != 3:
    raise ValueError("Este proyecto está configurado para simulaciones espaciales 3D (SPATIAL_DIM=3).")
nodes_in_xaxis = int(os.getenv('nodes_in_xaxis'))
nodes_in_yaxis = int(os.getenv('nodes_in_yaxis'))
nodes_in_zaxis = int(os.getenv('nodes_in_zaxis', os.getenv('nodes_in_yaxis')))
space_size = float(os.getenv('space_size'))
space_size_z = float(os.getenv('space_size_z', str(space_size)))
T = float(os.getenv('T'))
...
save_paraview = os.getenv('SAVE_PARAVIEW', 'Y').upper() == 'Y'
...
init_circle_center_z = float(os.getenv('INIT_CIRCLE_CENTER_Z', os.getenv('INIT_SPHERE_CENTER_Z', str(space_size_z / 2.0))))
init_circle_radius = float(os.getenv('INIT_CIRCLE_RADIUS', os.getenv('INIT_SPHERE_RADIUS', str(min(space_size, space_size_z) / 6.0))))
init_circle_edge_width = float(os.getenv('INIT_CIRCLE_EDGE_WIDTH', os.getenv('INIT_SPHERE_EDGE_WIDTH', '0.0')))
...
if init_profile == 'CIRCLE':
    ...
    print(
        "Condiciones iniciales: SPHERE "
        f"(center=({init_circle_center_x:.3g}, {init_circle_center_y:.3g}, {init_circle_center_z:.3g}), "
...
print(
    f"spatial_dim=3, mesh=({nodes_in_xaxis}, {nodes_in_yaxis}, {nodes_in_zaxis}), "
    f"domain=({space_size}, {space_size}, {space_size_z}), "
    f"sample_rate={sample_rate}, inner_iter={inner_max_iter}, inner_tol={inner_tol}"
)
...
paraview_dir = os.path.join(nueva_ruta, 'paraview')
...
if save_paraview:
    print(f"✓ ParaView VTI se guardará en: {paraview_dir}")
...
def _gather_coords_values_for_rank0(fenics_field):
    ...
    coords_local = domain.geometry.x[:, :3]
...
def sample_field_on_grid(fenics_field, space_size, sample_rate):
    ...
    Retorna (grid, x, y, z) en rank 0; en otros ranks retorna (None, None, None, None).
    """
    coords, values = _gather_coords_values_for_rank0(fenics_field)
    if comm.rank != 0:
        return None, None, None, None

    sample_points_x = np.linspace(0, space_size, int(space_size / sample_rate) + 1)
    sample_points_y = np.linspace(0, space_size, int(space_size / sample_rate) + 1)
    sample_points_z = np.linspace(0, space_size_z, int(space_size_z / sample_rate) + 1)
    X, Y, Z = np.meshgrid(sample_points_x, sample_points_y, sample_points_z, indexing='ij')
    grid = griddata(coords, values, (X, Y, Z), method='linear', fill_value=0.0)
    grid = np.nan_to_num(grid, nan=0.0, posinf=0.0, neginf=0.0)
    del X, Y, Z
    return grid, sample_points_x, sample_points_y, sample_points_z


def compute_field_stats(grid):
    """Estadísticas robustas de un grid 3D (numpy)."""
    if grid is None:
        return {}
    flat = grid.ravel()
    return {
...
def compute_gradient_stats(grid, h):
    ...
    if grid is None:
        return {}
    try:
        grads = np.gradient(grid, h, h, h, edge_order=1)
        gmag = np.sqrt(sum(g * g for g in grads))
        flat = gmag.ravel()
        return {
...
def save_diagnostic_snapshot(c, s, i, t, step, block, reason, extra=None):
    ...
    c_grid, x, y, z = sample_field_on_grid(c, space_size, diagnostic_sample_rate)
    s_grid, _, _, _ = sample_field_on_grid(s, space_size, diagnostic_sample_rate)
    i_grid, _, _, _ = sample_field_on_grid(i, space_size, diagnostic_sample_rate)
...
def create_space_function(space_size, nx, ny, nz):
    domain = mesh.create_box(
        comm,
        [np.array([0.0, 0.0, 0.0]), np.array([space_size, space_size, space_size_z])],
        [nx, ny, nz],
...
def field_to_numpy_array(fenics_field, space_size, step, field_name, block, sample_rate=0.02):
    field_array, x, y, z = sample_field_on_grid(fenics_field, space_size, sample_rate)
    if comm.rank != 0:
        return None
    ...
    os.makedirs(matrices_dir, exist_ok=True)
    filename = f"matrix_{field_name}_{step}_nb_{block}.npz"
    ...
    def _write_matrix_file():
        ...
        with open(tmp_path, "wb") as f:
            np.savez_compressed(
                f,
                field=field_array.astype(np.float32),
    ...
    return filepath, field_array, x, y, z


def save_paraview_vti(c_grid, s_grid, i_grid, x, y, z, step, block):
    """Guarda un volumen VTI con campos c, s, i para abrir directamente en ParaView."""
    if comm.rank != 0 or not save_paraview:
        return None

    nx, ny, nz = c_grid.shape
    spacing = (
        float(x[1] - x[0]) if len(x) > 1 else float(sample_rate),
        float(y[1] - y[0]) if len(y) > 1 else float(sample_rate),
        float(z[1] - z[0]) if len(z) > 1 else float(sample_rate),
    )
    origin = (float(x[0]), float(y[0]), float(z[0]))

    try:
        vtk_grid = pyvista.ImageData(dimensions=(nx, ny, nz), spacing=spacing, origin=origin)
    except AttributeError:
        vtk_grid = pyvista.UniformGrid(dimensions=(nx, ny, nz), spacing=spacing, origin=origin)

    vtk_grid.point_data["c"] = np.asarray(c_grid, dtype=np.float32).ravel(order="F")
    vtk_grid.point_data["s"] = np.asarray(s_grid, dtype=np.float32).ravel(order="F")
    vtk_grid.point_data["i"] = np.asarray(i_grid, dtype=np.float32).ravel(order="F")
    vtk_grid.field_data["t"] = np.array([float(step)], dtype=np.float64)
    vtk_grid.field_data["block"] = np.array([int(block)], dtype=np.int32)

    os.makedirs(paraview_dir, exist_ok=True)
    filepath = os.path.join(paraview_dir, f"fields_t_{step}_nb_{block}.vti")
    tmp_path = filepath + ".tmp.vti"
    ...
    def _write_vti_file():
        if os.path.isfile(tmp_path):
            try:
                ...
                pass
        vtk_grid.save(tmp_path)
        os.replace(tmp_path, filepath)

    _io_write_with_retries(
        _write_vti_file,
        f"paraview VTI t={step} nb={block}",
    )
    return filepath
...
def plot_fields(field_c, field_s, field_i, block, t, u_expr=None, V=None):
    ...
    z_slice = float(os.getenv("PLOT_Z_SLICE", str(space_size_z / 2.0)))
    coords_c = mesh_c.geometry.x[:, :3]
    values_c = field_c.x.array
    
    mesh_s = field_s.function_space.mesh
    coords_s = mesh_s.geometry.x[:, :3]
    values_s = field_s.x.array
    
    mesh_i = field_i.function_space.mesh
    coords_i = mesh_i.geometry.x[:, :3]
    ...
    X, Y = np.meshgrid(x, y)
    slice_points = np.column_stack([X.ravel(), Y.ravel(), np.full(X.size, z_slice)])

    # Cáncer
    plt.subplot(1, n_subplots, 1)
    Z_c = griddata(coords_c, values_c, slice_points, method='linear', fill_value=0.0).reshape(X.shape)
    p1 = plt.contourf(X, Y, Z_c, levels=20, cmap=_FIELD_CMAP_C)
    plt.colorbar(p1)
    plt.title(f'c at t = {t:.3f}, z={z_slice:.3f}')

    # Sanas
    plt.subplot(1, n_subplots, 2)
    Z_s = griddata(coords_s, values_s, slice_points, method='linear', fill_value=0.0).reshape(X.shape)
    p2 = plt.contourf(X, Y, Z_s, levels=20, cmap=_FIELD_CMAP_S)
    plt.colorbar(p2)
    plt.title(f's at t = {t:.3f}, z={z_slice:.3f}')

    # Inmune
    plt.subplot(1, n_subplots, 3)
    Z_i = griddata(coords_i, values_i, slice_points, method='linear', fill_value=0.0).reshape(X.shape)
    p3 = plt.contourf(X, Y, Z_i, levels=20, cmap=_FIELD_CMAP_I)
    plt.colorbar(p3)
    plt.title(f'i at t = {t:.3f}, z={z_slice:.3f}')
    ...
    if u_expr is not None and V is not None:
        ...
        Z_u = griddata(coords_u, values_u, slice_points, method='linear', fill_value=0.0).reshape(X.shape)
        p4 = plt.contourf(X, Y, Z_u, levels=20, cmap=_FIELD_CMAP_U)
        plt.colorbar(p4)
        plt.title(f'u at t = {t:.3f}, z={z_slice:.3f}')
    ...
    try:
        del X, Y, slice_points, Z_c, Z_s, Z_i
        if u_expr is not None and V is not None:
...
def _circle_weights_for_dofs(V, n_values):
    """
    Peso espacial de una esfera para cada DOF: 1 dentro, 0 fuera.
    ...
    dz0 = coords[:, 2] - init_circle_center_z
    radius = np.sqrt(dx0 * dx0 + dy0 * dy0 + dz0 * dz0)
    if init_circle_edge_width > 0.0:
...
def solve_dynamics():
    domain, V = create_space_function(space_size, nodes_in_xaxis, nodes_in_yaxis, nodes_in_zaxis)
    c = fem.Function(V)
    s = fem.Function(V)
    i = fem.Function(V)
...
# 2. Malla `P1` en `[0, space_size] x [0, space_size] x [0, space_size_z]`.
# 3. Resuelve el sistema sin control con iteraciones internas de Picard por paso (`inner_max_iter`, `inner_tol`).
# 4. Guarda `matrix_{c,s,i}_t_nb_block.npz` con volúmenes 3D muestreados.
...
if __name__ == "__main__":
    for block in range(1, nb + 1):
        ...
        while t < T + 1e-12:
            ...
            ts = f"{t:.3f}"
            c_saved = field_to_numpy_array(c, space_size, ts, 'c', block, sample_rate=sample_rate)
            s_saved = field_to_numpy_array(s, space_size, ts, 's', block, sample_rate=sample_rate)
            i_saved = field_to_numpy_array(i, space_size, ts, 'i', block, sample_rate=sample_rate)
            if comm.rank == 0 and c_saved and s_saved and i_saved:
                _, c_grid, x_grid, y_grid, z_grid = c_saved
                _, s_grid, _, _, _ = s_saved
                _, i_grid, _, _, _ = i_saved
                save_paraview_vti(c_grid, s_grid, i_grid, x_grid, y_grid, z_grid, ts, block)
                del c_grid, s_grid, i_grid, x_grid, y_grid, z_grid
