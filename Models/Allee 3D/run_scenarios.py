...
try:
    ...
    def get_results_dir(base_dir=None):
        raise RuntimeError(
            "No se pudo importar utils_paths.py; no se permite fallback local. "
            "Restaura utils_paths.py y monta Drive antes de ejecutar."
        )
...
def estimate_storage_per_scenario(common_params: Dict) -> float:
    ...
    sample_points_z = int(float(common_params.get('space_size_z', space_size)) / sample_rate) + 1
    matrix_size = sample_points * sample_points * sample_points_z
    bytes_per_matrix = matrix_size * 4  # float32 comprimido en NPZ para volúmenes 3D
    
    # Volúmenes por escenario (3 campos: c, s, i)
...
def validate_scenario_output(output_dir: Path, scenario: Dict, common_params: Dict) -> Tuple[bool, List[str]]:
    ...
    if matrices_dir.exists():
        matrix_files = list(matrices_dir.glob('matrix_*.npz'))
    ...
    if matrices_dir.exists() and len(list(matrices_dir.glob('matrix_*.npz'))) > 0:
...
def run_scenario(common_params, scenario, use_checkpoint=True, max_restarts=MAX_RESTARTS, checkpoint_max_step=None):
    ...
    try:
        ...
        print("  ✗ No se permite fallback local: monta Drive o define RESULTS_DIR apuntando a Drive.")
        return False
...
Ejemplos:
  ...
  python run_scenarios.py --no-pipeline-hint           # No imprimir comandos del pipeline al final
    ...
    if RESULTS_DIR.as_posix().startswith(drive_mount_point.as_posix()):
        ...
        error_msg = (
            f"Error: RESULTS_DIR no está bajo el punto de montaje de Google Drive ({drive_mount_point}). "
            f"Ruta resuelta: {RESULTS_DIR}"
        )
        print(f"✗ {error_msg}")
        print("  Monta Drive con: bash mount_google_drive.sh")
        print("  O define RESULTS_DIR apuntando a la carpeta de Drive.")
        logger.error(error_msg)
        return 1
