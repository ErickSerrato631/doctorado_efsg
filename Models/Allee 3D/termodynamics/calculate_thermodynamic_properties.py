...
def load_field_matrix(scenario_dir: Path, field_name: str, time: float, block: int = 1) -> Optional[np.ndarray]:
    ...
    # Formato 3D: matrix_{field_name}_{time:.3f}_nb_{block}.npz
    # Respaldo legacy 2D: matrix_{field_name}_{time:.3f}_nb_{block}.txt
    time_str = f"{time:.3f}"
    npz_path = matrices_dir / f"matrix_{field_name}_{time_str}_nb_{block}.npz"
    txt_path = matrices_dir / f"matrix_{field_name}_{time_str}_nb_{block}.txt"
    ...
    try:
        ...
        if txt_path.exists():
            return np.loadtxt(txt_path, dtype=float)
        return None
    except Exception as e:
        print(f"⚠ Error al cargar {npz_path if npz_path.exists() else txt_path}: {e}")
        return None
...
def get_available_times(scenario_dir: Path, field_name: str = 'c', block: int = 1) -> List[float]:
    ...
    times = []
    pattern = f"matrix_{field_name}_*_nb_{block}.npz"
    files = list(matrices_dir.glob(pattern))
    if not files:
        files = list(matrices_dir.glob(f"matrix_{field_name}_*_nb_{block}.txt"))
...
def calculate_gradient_nd(field: np.ndarray, dx: float) -> Tuple[np.ndarray, ...]:
    """
    Calcula el gradiente de un campo 3D usando diferencias finitas.
    También acepta arreglos 2D legacy para lectores antiguos.
    """
    if field.ndim not in (2, 3):
        raise ValueError(f"Se esperaba un campo 2D/3D, recibido shape={field.shape}")
    return tuple(np.gradient(field, dx, edge_order=1))
...
def calculate_gradient_2d(field: np.ndarray, dx: float) -> Tuple[np.ndarray, np.ndarray]:
    ...
    """
    grads = calculate_gradient_nd(field, dx)
    if len(grads) < 2:
        raise ValueError("El campo debe tener al menos dos dimensiones")
...
def integrate_spatial_density(density: np.ndarray, dx: float) -> float:
    """Integral espacial: en 3D usa sum * dx^3; en 2D legacy usa sum * dx^2."""
    return float(np.sum(density) * (dx ** density.ndim))
...
def calculate_thermodynamic_properties(
    ...
    # Necesitamos cargar una matriz/volumen para obtener el tamaño
    ...
    nx = sample_matrix.shape[0]
    dx = space_size / (nx - 1)  # Aproximación del espaciado
    
    print(f"Tamaño de campos: {sample_matrix.shape}, dx ≈ {dx:.6f}")
    ...
    for i, t in enumerate(times):
        ...
        cell_measure = dx ** c.ndim
        F_total = np.sum(F_data['F_total']) * cell_measure  # Integración espacial
        F_local = np.sum(F_data['F_local']) * cell_measure
        F_gradient = np.sum(F_data['F_gradient']) * cell_measure
        F_coupling = np.sum(F_data['F_coupling']) * cell_measure
