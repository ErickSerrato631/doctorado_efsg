...
DEFAULT_OUTPUT_DIR = _ALLEE_ROOT.parent.parent / "Paper" / "figures"
DEFAULT_SERIES_DIR = _ALLEE_ROOT / "results" / "nonequilibrium_time_series"
DEFAULT_T_MAX = 1.0
...
def _load_field_matrix_with_retries(
    ...
    filepath = matrices_dir / f"matrix_{field_name}_{float(t):.3f}_nb_{block}.txt"
    attempts = max(1, int(retry_attempts))
    ...
    for attempt in range(1, attempts + 1):
        try:
            ...
            print(
                f"WARN: I/O error reading {filepath.name} "
                f"(attempt {attempt}/{attempts}): {exc}",
                flush=True,
            )
            if attempt < attempts:
            ...
            print(f"WARN: failed reading {filepath}: {exc}", flush=True)
            return None
    print(f"WARN: giving up on {filepath.name} after {attempts} attempts", flush=True)
    return None
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    print(f"Results source: {results_dir if results_dir is not None else 'auto (RESULTS_DIR/Drive/local fallback)'}")
