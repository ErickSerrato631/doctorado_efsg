...
\begin{align}
\frac{dc}{dt} &= r_c c \left(\frac{c}{a} - 1\right)\left(1 - \frac{c}{k_c}\right) - c(\alpha s^2 + \beta i^2) - \mu c(\gamma s^2 + \eta i^2), \label{eqn:cancer_dynamic} \\
\frac{ds}{dt} &= r_s s \left(1 - \frac{s}{k_s}\right) - \gamma c^2 s + \delta i^2 s - \frac{\mu}{2} \alpha c^2 s, \label{eqn:sano_dynamic} \\
\frac{di}{dt} &= r_i i \left(1 - \frac{i}{k_i}\right) + \delta s^2 i - \eta c^2 i - \frac{\mu}{2} \beta c^2 i. \label{eqn:inmune_dynamic}
\end{align}
Las ecuaciones ODE anteriores son el límite homogéneo espacial del PDE (sin difusión), con la misma estructura de reacción local implementada numéricamente.
...
\begin{align}
\frac{dc}{dt} &= r_c c \left(\frac{c}{a} - 1\right)\left(\frac{c - a}{k_c - a}\right) - c(\alpha s^2 + \beta i^2) - \mu c(\gamma s^2 + \eta i^2), \\
\frac{ds}{dt} &= r_s s \left(1 - \frac{s}{k_s}\right) - \gamma c^2 s + \delta i^2 s - \frac{\mu}{2} \alpha c^2 s, \\
\frac{di}{dt} &= r_i i \left(1 - \frac{i}{k_i}\right) + \delta s^2 i - \eta c^2 i - \frac{\mu}{2} \beta c^2 i.
\end{align}
...
Los términos $-c\,\alpha s^2$, $-c\,\beta i^2$ y $-\eta c^2 i$ no son simétricos entre poblaciones: la supresión del sistema inmune por el tumor ($-\eta c^2 i$) difiere de la citotoxicidad inmune ($-c\,\beta i^2$), y el daño tisular ($-\gamma c^2 s$) difiere de la competencia contra el cáncer ($-c\,\alpha s^2$). Esta asimetría genera flujos no recíprocos que modifican la geometría del campo vectorial y la ubicación de umbrales.
